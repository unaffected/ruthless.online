...
export function EnergyWidget() {
  const [energy, setEnergy] = useState({ current: 100, maximum: 100 })
  
  useGameEvent('client:player:stats', (data) => {
    setEnergy({
      current: data.energy_current,
      maximum: data.energy_maximum
    })
  })
  
  const percentage = Math.max(0, Math.min(100, (energy.current / energy.maximum) * 100))
  ...
  return (
    <div className="flex flex-col gap-1.5 w-[200px]">
      ...
      <div className="relative h-6 border border-white/15 rounded overflow-hidden">
        ...
        >
          {Math.ceil(energy.current)} / {energy.maximum}
        </div>
      </div>
    </div>
  )
